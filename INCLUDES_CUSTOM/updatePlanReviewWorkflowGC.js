...
function updatePlanReviewWorkflowGC(wfTask, status, wfComments, updateIndicator) {
	...
	if (updateIndicator == "I") {
		...
		if (!matches(action, "", undefined)) {
			var taskItemResult = aa.workflow.getTasks(capId).getOutput();
			for (i in taskItemResult) {
				fTask = taskItemResult[i];
				if (fTask.getTaskDescription().equals(wfTask)) processName = fTask.getProcessCode();
			}
			if (action == "Next") {
				closeTask(wfTask, status, wfComments, "", processName);
				return true;
			} else if (action == "No Change") {
				updateTask(wfTask, status, wfComments, "", processName);
				return true;
			} else if (action == "Loop") {
				loopTask(wfTask, status, wfComments, "", processName);
				return true;
			} else if (action == "Branch") {
				branchTask(wfTask, status, wfComments, "", processName);
				return true;
			} else {
