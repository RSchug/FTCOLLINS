// WorkflowTaskUpdateAfter4Renew.txt  TruePoint
// Last updated 5/4/2014 by CP
	...
	aa.env.getValue("WorkflowStatus"))) //task status
{
	...
	var parentLicenseCAPID = getParentCapIDForReview(capID)
        logDebug("The Renewal is approved, Task = Closed or Renewal Review, Status = Issue Renewal");
...
if (debug.indexOf("**ERROR") > 0)
	...
	{
	aa.env.setValue("ScriptReturnCode", "0");
	if (showMessage) aa.env.setValue("ScriptReturnMessage", "WorkflowTaskUpdateAfter for Renewal process.");
...
function isWorkflowApproveForReview(capID, wfTask, stepNum, processID, taskStatus)
{
    ...
    if(result.getSuccess())
	{
		...
		if (taskItemScriptModel.getTaskDescription().equals(wfTask)
			&& ("Renewal Application".equals(wfTask) || "Renewal Review".equals(wfTask) ||"Closed".equals(wfTask)||"Renewal Inspection".equals(wfTask)|| "License Renewal".equals(wfTask))
			&& ("Issue Renewal".equals(taskStatus)|| "Complete".equals(taskStatus) || "Renewal Issued".equals(taskStatus) || "Renewed - Payment Pending".equals(taskStatus) ||"Renewed".equals(taskStatus)))
		{
			return true;
			logDebug("Issuing license (" + wfTask +") has been approved in isWorkflowApproveForReview");
		}	
		else
		{
			logDebug("Issuing license (" + wfTask +") has not been approved in isWorkflowApproveForReview");
		}
	}  
    else 
    {
...
function isWorkflowDenyForReview(capID, wfTask, stepNum, processID, taskStatus)
{
    ...
    if(result.getSuccess())
	{
		...
		//2. Check to see if the agency user approve renewal application .
		if (taskItemScriptModel.getTaskDescription().equals(wfTask)
			&& "Closed".equals(wfTask)
			&& "Issue Renewal".equals(taskStatus))
		{
			return true;
		}	
		else
		{
			logDebug("Issuing license (" + wfTask +") has not been approved in isWorkflowDenyForReview");
		}
	}  
    else 
    {
...
function isReadyRenew(capid)
{
    ...
    if(result.getSuccess())
	{
           logDebug("True: from isReadyRenew");
           return true;
...
function getRenewalCapByParentCapIDForReview(parentCapid)
{
	...
	//1. Get parent license for review
	var result = aa.cap.getProjectByMasterID(parentCapid, "Renewal", "Incomplete");//"Incomplete" was "Review" 
    if(result.getSuccess())
	{
		projectScriptModels = result.getOutput();
		if (projectScriptModels == null || projectScriptModels.length == 0)
...
function ApproveAuthorizedTransaction(capID)
{
	...
	{
		aa.log("No authorized transaction found for this CAP: " + entityID);
		return false;
	}
