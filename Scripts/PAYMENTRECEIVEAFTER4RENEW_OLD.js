...
{
	// 2. Check to see if license is ready for renew.
		...
		if (result.getSuccess())
		{
			...
			if (projectScriptModel.RENEWAL_COMPLETE.equals(projectScriptModel.getStatus()))
			{
				...
				if (activeLicense(parentLicenseCAPID))
				{
					...
					//5.1.2. Send auto-issurance license email to public user
...
function getParentLicenseCapID(capid)
{
    ...
    if(result.getSuccess())
	{
		...
		partialCAPID = projectScriptModel.getProjectID();
		...
		{
			aa.print("ERROR: Failed to  parent license with CAPID(" + projectScriptModel.getProjectID() + "): " + result.getErrorMessage());
			return null;
		}
	}  
    else 
