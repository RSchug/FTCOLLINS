...
| Action# : N/A
| 
| Notes   :  Modified 3/19/08  Added capIdArrayString in getRelatedCapsByAddress function //  Kristi Kreisher
|
|**CONTAINS CUSTOM CODE IN UpdateChildTasks Function
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var capIdArrayString = "";						// alternate cap ids for all caps in array		
...
logDebug("seconds: " + addSecondsVariable);
/*------------------------------------------------------------------------------------------------------/
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			var dispositionDate = aa.date.getCurrentDate();
			addSecondsVariable++;
			dispositionDate.setSecond(dispositionDate.getSecond()+addSecondsVariable);
			var stepnumber = fTask.getStepNumber();
			var processID = fTask.getProcessID();
...
function getAppIdByASI(ASIName,ASIValue,ats)
	...
	for (aps in apsArray)
		...
		myrelated = myCap.getCapType().toString();
...
{					
retArr.push(capIdArray[cappy]);			
var CaseIDResult = capIdArray[cappy].getCustomID();			
capIdArrayString = CaseIDResult + ", " + capIdArrayString;				
}			
			} // loop through related caps

logDebug("Prior Case Numbers are:  " + capIdArrayString);
	}
	...
	var childResult = aa.cap.getChildByMasterID(capId);  // children array
 	...
 	if (childResult.getSuccess())
  	 	childArray = childResult.getOutput();
  	  	...
  	  	{ message+="ERROR: Failed to get child apps: " + childResult.getErrorMessage() + br; return false; }
