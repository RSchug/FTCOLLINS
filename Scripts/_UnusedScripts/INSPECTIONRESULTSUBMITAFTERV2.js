/*------------------------------------------------------------------------------------------------------/
| Program : InspectionResultSubmitAfterMasterScript  
...
logDebug("capDetail = " + capDetail.getClass());
...
var InspectionType = aa.env.getValue("InspectionType");
var InspectorFirstName = aa.env.getValue("StaffFirstName");
var InspectorLastName = aa.env.getValue("StaffLastName");
var InspectorMiddleName = aa.env.getValue("StaffMiddleName");
var InspectionResult = aa.env.getValue("InspectionResult");

logDebug("InspectionResult = " + InspectionResult);
logDebug("InspectionType = " + InspectionType);
...
function appHasCondition(pType,pStatus,pDesc,pImpact)
	...
	if (condResult.getSuccess())
		...
		{ 
		logMessage("ERROR","ERROR: getting cap conditions: " + condResult.getErrorMessage());
	...
	for (cc in capConds)
		{
		thisCond = capConds[cc];
		cStatus = thisCond.getConditionStatus();
		cDesc = thisCond.getConditionDescription();
		cImpact = thisCond.getImpactCode();
		if (cStatus==null)
			cStatus = " ";
		if (cDesc==null)
			cDesc = " ";
		...
		if ( (pStatus==null || pStatus.toUpperCase()==cStatus.toUpperCase()) && (pDesc==null || pDesc.toUpperCase()==cDesc.toUpperCase()) && (pImpact==null || pImpact.toUpperCase()==cImpact.toUpperCase()) )
...
//
	...
	for (zzz in aoArray)
		...
		for (cappy in capIdArray)
			...
			aa.print(capId.getCustomID() + " : " + capIdArray[cappy].getCustomID());
			// get cap type
...
function inspCancelAll()
	...
	if (inspResults.getSuccess())
		...
		logMessage("ERROR", "ERROR: getting inspections: " + inspResults.getErrorMessage());
...
function loadTasks(ltcapidstr)
	...
	var ltresult = aa.cap.getCapID(ltcapidstr); 
 	...
 	if (ltresult.getSuccess())
  	 	var ltCapId = ltresult.getOutput();
  	else
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice) // Updates an assessed fee with a new Qty.  If it is not found, adds it
	...
	if (getFeeResult.getSuccess())
		...
		for (feeNum in feeList)
			...
			if (feeList[feeNum].getFeeitemStatus().equals("INVOICED"))
				{
				logDebug("Invoiced fee "+fcode+" found, not updated");
				feeInvoiced = true;
				}
			}
		}		
	...
	if (!feeUpdated && !feeInvoiced) // no existing fee, so add it
