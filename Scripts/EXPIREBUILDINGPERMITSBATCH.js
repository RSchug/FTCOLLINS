/*------------------------------------------------------------------------------------------------------/
| Program: ExpireBuildingPermitsBatch	Trigger: Batch    Client : Spokane   SAN# : 06SSP-00242
| 
| SPOKANE 06SSP-00242
|                                                                       
...
| Version 0.4 - Only expires permit if it is NOT in specific statuses:  Completed, Closed, Expired, Issued TCO, Revoked, Withdrawn, Void	05/24/2018 - Kristi Kreisher
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
if (ASIResult.getSuccess())
	...
	for (ii in ASIArray)
		...
		if (capStatus && !capStatus.equals("Completed")&& !capStatus.equals("Closed") && !capStatus.equals("Issued TCO") && !capStatus.equals("Revoked") && !capStatus.equals("Withdrawn") && !capStatus.equals("Void") && !capStatus.equals("Expired"))
			updateAppStatus(statusSet,"Auto-expiration by batch script");
			
		else
			{
			logMessage(capId.toString() + ": Already at " + capStatus + " status");
			}
